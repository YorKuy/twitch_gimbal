...
#define MINI_PC_USART_HANDLE 		huart2// 串口号，可修改
...
#define SP_HEADER           0x66
#define SP_TAIL             0x11
#define SP_TOTAL_LEN        29

#define SP_OFFSET_YAW       2
#define SP_OFFSET_YAW_VEL   (SP_OFFSET_YAW + F_Byte * 1)
#define SP_OFFSET_YAW_ACC   (SP_OFFSET_YAW + F_Byte * 2)
#define SP_OFFSET_PITCH     (SP_OFFSET_YAW + F_Byte * 3)
#define SP_OFFSET_PITCH_VEL (SP_OFFSET_YAW + F_Byte * 4)
#define SP_OFFSET_PITCH_ACC (SP_OFFSET_YAW + F_Byte * 5)                                                  
...
struct __attribute__((packed)) GimbalToVision  //新自瞄数据结构体
{
  uint8_t head[2] = {'S', 'P'};
...
static inline void UnpackFloatSafe_Zm(volatile const uint8_t *buf, uint32_t offset, volatile FloatBytes_t *target,float Safe_Angle)
{
    UnpackFloat(buf,offset,target);
    if(target->f > 360.0f || target->f < -360.0f || isnan(target->f)) {
        target->f = Safe_Angle;
...
extern volatile _type_zm  AS;
extern _request_union request_union;
extern volatile _SuperPower SuperPower;
