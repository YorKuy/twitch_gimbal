...
volatile static uint8_t IMU_FIFO = 0;
void IMU_UART_Init(void)
{
    /* 清零结构体 */
    memset(&hipnuc_raw, 0, sizeof(hipnuc_raw_t));
    memset(&imu_data, 0, sizeof(hi91_t));
    __HAL_UART_ENABLE_IT(&IMU_UART_HANDLE, UART_IT_IDLE);
    // __HAL_DMA_DISABLE_IT(&hdma_uart4_rx, DMA_IT_HT); // 可选：禁用半传输中断
    HAL_UART_Receive_DMA(&IMU_UART_HANDLE, imu_dma_rxbuf[0], IMU_DMA_BUF_SIZE);
	HAL_UART_Transmit(&IMU_UART_HANDLE, (uint8_t*)"REBOOT\r\n", strlen("REBOOT\r\n"),0xFFFF);
    HAL_Delay(1000);
}
void IMU_UART_IRQHandler(DMA_HandleTypeDef *hdma)
{
    ...
    if ((tmp_flag != RESET))
    {
        ...
        IMU_rx_size = IMU_DMA_BUF_SIZE - temp;
